...
export const listCustomer = async (req, res) => {
    try {
        ...
        let datas = await User.find({ isDeleted: false, role: '6512c4c6185c0a6bf02b2c65', createdBy: whoAmI?.email }).select("-password");
...
export const customerDetails = async (req, res) => {
    try {
        ...
        let userInfo = null;
        try {
            userInfo = await User.findOne({ _id: req?.body?.id, role: "6512c4c6185c0a6bf02b2c65", createdBy: whoAmI?.email, isDeleted: false }).select("-password");
        } catch (err) {
...
export const updateCustomer = async (req, res) => {
    try {
        ...
        const userInfo = await User.findOne({ _id: req?.body?.id, createdBy: whoAmI?.email, isDeleted: false });
...
export const deleteCustomer = async (req, res) => {
    try {
        let whoAmI = await authValues(req.headers['authorization']);
        let customerInfo = await User.findOne({ _id: req?.params?.id, role: "6512c4c6185c0a6bf02b2c65", createdBy: whoAmI.email, isDeleted: false });
